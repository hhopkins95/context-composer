...
const TreeNode = ({ node, level, onNodeSelect, onAddChild, collapsedNodes, onToggleCollapse, onDeleteNode, onMoveNode }: {
  node: Node
  level: number
  onNodeSelect: (node: Node) => void
  onAddChild: (parentId: string) => void
  ...
  return (
    <div ref={ref} className={`pl-4 ${isDragging ? 'opacity-50' : ''} ${isOverCurrent ? 'bg-gray-100' : ''}`}>
      <div className={`flex items-center space-x-2 cursor-pointer hover:bg-gray-100 p-1 rounded ${colorClass}`}>
        ...
        <span onClick={() => onNodeSelect(node)}>
          ...
          {node.tagName}: {node.content.substring(0, 20) || `[Empty ${node.type}]`}
        </span>
        <Button 
          size="sm" 
          variant="ghost" 
...
export default function TreeView({ nodes, onNodeSelect, onAddChild, collapsedNodes, onToggleCollapse, onDeleteNode, onMoveNode }: TreeViewProps) {
  return (
    <div className="p-4">
